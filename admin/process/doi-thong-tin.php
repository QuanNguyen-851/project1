...
if (isset($_POST['ten']) && isset($_POST['gt']) && isset($_POST['date']) && isset($_POST['Email']) && isset($_POST['sdt']) && isset($_POST['address'])) {
	...
	if ($check1==0 || $e == $email) {
		if ($check2==0 || $s ==$sdt) {
			...
			if (isset($_GET['ma'])) {
				header("location:../common/main.php?command=1&ma=$ma&ok");
			} else {
				header("location:../common/main.php?command=1&ok");
			}
		} else {
			if (isset($_GET['ma'])) {
				header("location:../common/main.php?command=1&ma=$ma&sdt");
			} else {
				header("location:../common/main.php?command=1&sdt");
			}
		}
		} else {
			if (isset($_GET['ma'])) {
				header("location:../common/main.php?command=1&ma=$ma&email");
			} else {
				header("location:../common/main.php?command=1&email");
			}
		}
	} else {
		if (isset($_GET['ma'])) {
				header("location:../common/main.php?command=1&ma=$ma&loi");
			} else {
				header("location:../common/main.php?command=1&loi");
			}
	}
