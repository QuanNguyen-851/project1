...
if(isset($_POST['new-pass']) &&  isset($_POST['current-pass']) && isset($_GET['ma'])){
	...
	while ($mk = mysqli_fetch_array($result)){
		if($old == $mk['pass']){
			...
			header("location:main.php?command=1&success=1");
		} else {
			header("location:main.php?command=1&success=0");
		}
	}
	...
	header("location:main.php?command=1&ma=$ma&success=1");
} else {
	if (isset($_GET['quyen']) && isset($_GET['ma'])) {
		$ma = $_GET['ma'];
		header("location:main.php?command=1&tab=1&fail&ma=$ma");
	} else {
	header("location:main.php?command=1&tab=1&fail");
}
}
